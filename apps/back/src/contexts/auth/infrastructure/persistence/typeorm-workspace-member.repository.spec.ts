...
describe('TypeOrmWorkspaceMemberRepository.insertFounder', () => {
  it.each([
    ...
    'maps %s to a field-specific unique constraint',
    async (constraint, field, value, googleSubject) => {
      const insert = jest.fn().mockRejectedValue(uniqueViolation(constraint));
      const repository = new TypeOrmWorkspaceMemberRepository({
        insert,
      } as unknown as Repository<WorkspaceMemberOrmEntity>);
...
describe('WorkspaceMemberMapper', () => {
  it.each(['admin', 'member'] as const)(
    'preserves the explicit %s role in both directions',
    (role) => {
      const orm = WorkspaceMemberMapper.toOrm(buildMember(null, role));

      expect(orm.role).toBe(role);
      expect(WorkspaceMemberMapper.toDomain(orm).getRole()).toBe(role);
    },
  );
});
